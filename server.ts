...
process.on("uncaughtException", (err) => {
  logSafeError("UNCAUGHT EXCEPTION:", err);
  process.exit(1);
});
process.on("unhandledRejection", (err) => {
  logSafeError("UNHANDLED REJECTION:", err);
  process.exit(1);
});

const app = express();

// Security: never expose raw SDK/server errors to clients or logs.
// In particular, provider errors can sometimes contain request metadata.
...
let serverStartPromise: Promise<HttpServer> | null = null;
...
async function verifyUserAuth(req: AuthenticatedRequest, res: express.Response, next: express.NextFunction) {
  try {
    ...
    const { plainText: plainKey, isLegacy } = decryptApiKey(encryptedKey);
    ...
    // Auto-migrate legacy record: re-encrypt using ENCRYPTION_SECRET and update Firestore
    if (isLegacy) {
      try {
        const reEncryptedKey = encryptApiKey(plainKey);
        await updateFirestoreDoc("users", userInfo.uid, {
          encryptedApiKey: reEncryptedKey,
    ...
    logSafeError("Auth Verification Error:", error);
...
app.post("/api/user/save-key", async (req, res) => {
  try {
    ...
    logSafeError("Error saving API Key:", error);
...
app.post("/api/user/remove-key", async (req, res) => {
  try {
    ...
    logSafeError("Error removing API Key:", error);
...
app.get("/api/user/available-models", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logSafeError("Error in available-models:", error);
...
app.post("/api/user/model-settings", async (req, res) => {
  try {
    ...
    logSafeError("Error saving model settings:", error);
...
app.post("/api/user/delete-account", async (req, res) => {
  try {
    ...
    logSafeError("Error deleting user account:", error);
...
app.post("/api/image-studio/generate", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!apiKey) {
      return res.status(403).json({ error: "API key required for image generation. Please add your API Key in Settings." });
    }
    ...
    if (req.userAuth?.uid && req.headers.authorization) {
      ...
      try {
        ...
        logSafeError("Failed to save image history to Firestore:", histErr);
      }
    }

    return res.json({ success: true, images });
  } catch (error: any) {
    logSafeError("Image generation error:", error);
...
`;
    ...
    logSafeError("Enhance prompt error:", error);
...
app.get("/api/image-studio/history", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logSafeError("Get image history error:", error);
...
app.delete("/api/image-studio/history/:imageId", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logSafeError("Delete image history error:", error);
...
`;
    ...
    logSafeError("Error in /api/generate:", error);
...
app.post("/api/generate-topic", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logSafeError("Error in /api/generate-topic:", error);
    res.status(500).json({ error: "An error occurred generating from topic." });
  }
});
...
Return ONLY the clean, word-by-word spoken transcript text with NO speaker tags, timestamps, meta-introductions, descriptions, or surrounding commentary. Start directly with the spoken dialogue.`;
    ...
    try {
      ...
      try {
        ...
        logSafeError("Transcript fallback failed:", fallbackErr);
      }
    }
    ...
    logSafeError("Error in /api/extract-transcript:", error);
...
app.post("/api/parse-file", verifyUserAuth as express.RequestHandler, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logSafeError("Error in /api/parse-file:", error);
    res.status(500).json({ error: "An error occurred parsing the file." });
  }
});
...
`;
    ...
    logSafeError("Error in /api/generate-scenes:", error);
    res.status(500).json({ error: "An error occurred generating scenes." });
  }
});
...
`;
    ...
    logSafeError("Error in /api/regenerate-scene:", error);
    res.status(500).json({ error: "An error occurred regenerating the scene." });
  }
});
...
`;
    ...
    logSafeError("Error in /api/generate-ctr:", error);
    res.status(500).json({ error: "An error occurred generating CTR growth assets." });
  }
});
...
`;
    ...
    logSafeError("Error in /api/generate-thumbnail-prompt:", error);
    res.status(500).json({ error: "An error occurred generating thumbnail prompt." });
  }
});
...
/**
 * Starts the local Express API used by both the web app and Electron.
 *
 * Electron loads the packaged server in-process, so this function is exported
 * and its promise is kept available for the Electron main process to await.
 * The normal `node dist/server.cjs` / Vite workflow still starts automatically.
 */
export async function setupServer(): Promise<HttpServer> {
  ...
  serverStartPromise = (async () => {
    if (process.env.NODE_ENV !== "production") {
      ...
      // In Electron, __dirname points to the packaged server bundle directory.
      // This is intentionally NOT process.cwd(), because Electron's working
      // directory can be outside the application's resources directory.
    ...
    httpServer = await new Promise<HttpServer>((resolve, reject) => {
      ...
      const onListening = () => {
        ...
        console.log(
          `Server running at http://${HOST}:${PORT} with environment ${process.env.NODE_ENV || "development"}${
            isElectron ? " (Electron)" : ""
          }`
        );
        resolve(server);
...
/** Allows the Electron main process to shut down the local API cleanly. */
...
setupServer().catch((err) => {
  logSafeError("FAILED TO START SERVER:", err);
  process.exit(1);
});